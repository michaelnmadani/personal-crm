-- Fix: re-apply merge_contacts so emails/phones can never become NULL.
-- When both contacts have empty emails/phones, jsonb_agg(distinct ...) returns
-- NULL, which violated the not-null constraint on contacts.emails and made every
-- merge fail. Wrapping the aggregate in coalesce(..., '[]'::jsonb) fixes it.
-- (The live DB had an older definition than 0006; this migration realigns it.)
--
